...
import { ChevronLeftIcon, MenuIcon, PanelLeftIcon } from 'lucide-react';
...
function Sidebar({
	...
	if (isMobile) {
		return (
			<>
				<header className="bg-background/80 border-b-border z-header fixed top-0 left-0 w-full border-b p-2 backdrop-blur-md">
					<SidebarTrigger iconType="menu" />
				</header>
				<Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
					<SheetContent
						...
						data-slot="sidebar"
						data-mobile="true"
						className="z-sidebar bg-sidebar text-sidebar-foreground w-(--sidebar-width) p-0 [&>button]:hidden"
						style={
							{
...
function SidebarTrigger({
	...
	iconType = 'panel',
	...props
}: React.ComponentProps<typeof Button> & { iconType?: 'menu' | 'panel' }) {
...
function SidebarGroupContent({
	...
	return (
		<div
			...
			className={cn('t-body-3 w-full', className)}
			{...props}
		/>
	);
}
...
const sidebarMenuButtonVariants = cva(
	'peer/menu-button t-body-3 ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-medium [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
	{
		variants: {
			...
			size: {
				default: 't-body-3 h-10',
				sm: 'h-7 text-xs',
				lg: 't-body-3 h-12 group-data-[collapsible=icon]:p-0!',
			},
		},
		defaultVariants: {
			variant: 'default',
